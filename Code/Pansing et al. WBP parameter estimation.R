...
SEED_germ_ERP <- survival %>% 
  group_by(., study.area) %>% 
  summarise_at(., vars(germ.2013.y.n), funs(mean))
...
SEED2_germ <- SEED2 %>% 
  group_by(., study.area) %>% 
  summarise_at(., vars(germ.2014.y.n), funs(mean))   #get mean germination rate for both study areas
...
source(paste0(proj_dir,code_dir,"WBP Seedling Survival Estimates.R"))
